...
- Content requests use a browser-like mobile `User-Agent` so Klipy can return ad inventory correctly, and the SDK also sends `X-Klipy-Client: klipy-ios-sdk/1.3.2 (iOS; community SDK)` so traffic still identifies itself as coming from this community-maintained iOS package.
