...
final class KlipyClientContentQueryTests: XCTestCase {
    ...
    func testSearchAddsAdIframeAndAdContextToContentRequests() async throws {
        ...
        mock.onRequestHandler = OnRequestHandler(httpBodyType: Data.self) { request, _ in
            ...
            XCTAssertEqual(query["ad-max-height"], "200")
            XCTAssertEqual(query["ad-os"], "ios")
            XCTAssertEqual(query["ad-model"], "iphone")
            XCTAssertNotNil(query["ad-device-w"])
            XCTAssertNotNil(query["ad-device-h"])
            XCTAssertNotNil(query["ad-pxratio"])
